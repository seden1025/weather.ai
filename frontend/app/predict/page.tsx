    ...
    <li className="rounded-xl border border-gray-200 p-4 text-sm">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <span className="text-gray-500">
          {p.predict_date} · {p.sky_condition} · 바람 {p.wind_feel}
        </span>
        {winner && (
          <span
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-center">
        <div>
          <p className="text-xs text-gray-400">내 예측</p>
          <p className="font-semibold">{p.predicted_max_temp}°C</p>
        </div>
        <div>
          <p className="text-xs text-gray-400">AI 예측</p>
...
export default function PredictPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(false);
    if (!prediction) {
      setError("제출에 실패했습니다. 잠시 후 다시 시도해주세요 (서버가 방금 깨어난 경우 최대 1분 걸릴 수 있어요).");
      return;
    }
    ...
    <main className="mx-auto max-w-3xl space-y-8 p-8">
      <header className="space-y-2 border-b border-gray-200 pb-4">
        ...
        <p className="text-sm text-gray-600">
          창문 밖을 직접 내다보고 하늘 상태와 바람을 관찰한 뒤, 오늘의 최고기온을
          예측해보세요. 같은 순간 AI 모델도 똑같이 예측합니다 — 사람의 직관과
          AI 중 누가 더 정확할지 비교해보는 게 이 프로젝트의 원래 출발점이었어요.
          저녁에 다시 와서 실제 결과와 비교해보세요.
        </p>
      </header>
          ...
          <div className="grid grid-cols-2 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">
                {result.ai_predicted_max_temp != null ? `${result.ai_predicted_max_temp}°C` : "모델 없음"}
              </p>
            </div>
          </div>
          <p className="mt-3 text-center text-xs text-gray-500">
            오늘 하루가 지나면 실제 최고기온과 비교해서 누가 더 정확했는지 아래 기록에서 확인할 수 있어요.
          </p>
        </section>
      )}
